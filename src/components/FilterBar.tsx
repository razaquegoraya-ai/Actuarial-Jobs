...
export default function FilterBar({ filters, onFilterChange, availableLocations, availableTags }: FilterBarProps) {
  const handleReset = () => {
    onFilterChange({
      keyword: '',
      job_type: 'All',
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pt-4 border-t border-gray-200">
        <div className="flex items-center gap-4">
          ...
          <select
            id="sort"
            value={filters.sort}
            onChange={(e) => onFilterChange({ ...filters, sort: e.target.value as 'date_desc' | 'date_asc' })}
